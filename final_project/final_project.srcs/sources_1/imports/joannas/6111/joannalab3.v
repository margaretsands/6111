...
module lab3   (beep, audio_reset_b, ac97_sdata_out, ac97_sdata_in, ac97_synch,
   ...
   assign reset = user_reset | power_on_reset;
	...
	wire switch_sides;
	debounce db4(.reset(reset),.clock(clock_65mhz),.noisy(~button0),.clean(switch_sides));
   ...
   pong_game pg(.vclock(clock_65mhz),.reset(reset),
		...
		.hcount(hcount),.vcount(vcount), .switch_sides(switch_sides),
                .hsync(hsync),.vsync(vsync),.blank(blank),
...
module pong_game (
   ...
   assign pblank = blank;
	...
	picture_blob #(.WIDTH(death_width), .HEIGHT(death_height)) blob1(.pixel_clk(vclock),.x(x),.y(y),.hcount(hcount),.vcount(vcount),.pixel(death_pixel));
	...
	wire [23:0] red_pixel;
	blob #(.WIDTH(10'd128),.HEIGHT(10'd128),.COLOR(24'hFF_00_00))   // yellow!
     red_square(.x(screen_width/2 - 64),.y(screen_height/2-64),.hcount(hcount),.vcount(vcount),
             .pixel(red_pixel));			 
	
	assign pixel = ({((red_pixel[23:16]*(alpha_n-alpha_m)+death_pixel[23:16]*alpha_m) >> alpha_log_n), (death_pixel[15:8]*alpha_m >> alpha_log_n), (death_pixel[7:0]*alpha_m) >> alpha_log_n} + paddle_pixel);
	always @ (posedge vclock) begin
...
module picture_blob
   ...
   DeathStarColor  rom1(.clka(pixel_clk), .addra(image_addr), .douta(image_bits));

   // use color map to create 8 bits R, 8 bits G, 8 bits B
   // since the image is greyscale, just replicate the red pixels
   // and not bother with the other two color maps.
   // use color map to create 8bits R, 8bits G, 8 bits B;
   ColorMapDeathStar rcm (.clka(pixel_clk), .addra(image_bits), .douta(red_mapped));
